...
async def add_grocery_list(grocery_list: dict, current_user: dict = Depends(get_current_user)):
    try:
...
async def get_grocery_list(week: int, current_user: dict = Depends(get_current_user)):
    grocery_list = await db['grocery_list'].find_one({"week": week, "user_id": current_user['_id']})
    if not grocery_list:
...
async def get_week(current_user: dict = Depends(get_current_user)):
    try:
        documents = db['grocery_list'].find({"user_id": current_user['_id']}).sort({"week": -1})
        weeks = []
